        ...
        <article className="w-full overflow-hidden rounded-lg shadow-lg p-4 h-4/6 bg-cover" style={{ backgroundImage: `${letter?.bg ? `url(${letter.bg})`: `url(https://picsum.photos/600/400/?random)`}` }}>
            <div className="flex flex-col items-center justify-between  h-full">
                <div className='mt-10'>
                   ...
                   {isComplete &&  <input type="text" className='text-sm p-2 text-center placeholder-gray-600 font-mono focus:outline-none bg-transparent border-b-2 border-gray-500' defaultValue={letter?.to || ''}  />}
                </div>     
                ...
                {isComplete &&  <textarea className="w-full p-2 bg-transparent outline-none placeholder-gray-600 resize-none h-4/5"  placeholder="여기부터 편지를 써주세요" defaultValue={letter?.content || ''} ></textarea>}   
            </div>
        </article>
    );
}
